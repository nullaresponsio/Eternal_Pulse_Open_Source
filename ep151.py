...
- Microarchitectural attack payloads
...
class EternalPulseScanner:
    ...
    def _differential_fuzz(self, host: str, port: int):
        ...
        try:
            # Probe SMBv2
            v2_response = self._probe_smb_dialect(host, port, b"\x02\x02")
            # Probe SMBv3
            v3_response = self._probe_smb_dialect(host, port, b"\x02\x28")
            ...
            if v2_response[68:70] != v3_response[68:70]:
                self._log(f"[DIFFERENTIAL][{host}] Different dialects negotiated")
            
            # Check for crash-inducing differences
            if not v2_response or not v3_response:
                with self.crash_lock:
                    self.crash_counter += 1
                self._log(f"[CRASH][{host}] Differential fuzzing caused crash")
        except Exception as e:
            self._log(f"[DIFFERENTIAL-ERROR][{host}] {str(e)}")

    def _probe_smb_dialect(self, host: str, port: int, dialect: bytes) -> bytes:
        """Probe specific SMB dialect"""
        with socket.socket() as sock:
            sock.settimeout(self.timeout)
            sock.connect((host, port))
            payload = b"\x00\x00\x00\xc0\xfeSMB\x00" + os.urandom(32) + b"\x24\x00\x01\x00" + dialect
            payload = self.evasion_engine.apply_evasion(payload, host)
            sock.sendall(payload)
